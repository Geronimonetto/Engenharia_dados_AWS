...
def count_page() -> list:
    ...
    :return: dados
    """
    url = f'https://api.themoviedb.org/3/discover/tv?page=1&api_key={api_key}&with_genres={gender}&language=en'  # URL da 1° página
...
def send_data(json_filename: str, type_gender_func: str, number_file: int) -> None:
    ...
    s3.upload_file(json_filename, bucket_name,
                   f"Raw/TMDB/JSON/Series/2023/10/09/{type_gender_func}/data_{number_file}.json")
...
def run_data(page_number: list) -> None:
    ...
    for count in page_number:
        url = f'https://api.themoviedb.org/3/discover/tv?page={count}&api_key={api_key}&with_genres={gender}'
